...
`GET /admin` 返回根目录 `admin.html`，Toy 首页登录区和移动端弹窗会在管理员账号登录后显示入口。
...
| `ask` | `room_id`, `content`, `path_token?` | 调用海龟汤 `ask` 逻辑，`content` 最多 200 字，受 AI 冷却限制；若房间已结束返回「房间已结束，无法继续提问」 |
...
| `/soup/room/:id` | `Room` | 对局页：SSE 订阅、`presence.enter_room`；判定前缀中文展示；开小差类日志归一为 `【系统提示】...`；`auto_hint`→「【线索公布】」、`hint_offer`→「【请求提示】」（每个玩家每房间 3 次，只有请求者能接受/拒绝，请求前确认）；`game_over`→「汤底揭晓」且正文含还原度；记事本侧栏/抽屉；若 ask/guess 返回 `system_error` 则保留输入框内容 |
...
Room 移动端规则：顶部栏合并标题和状态；汤面区高度压缩并可一键折叠，右侧滚动条常显且加宽加深；底部提问/猜测汤底输入区、请求提示按钮与发送按钮压缩高度。侦探日志标题条在移动端只保留「侦探日志」文字并压低高度；日志时间列与判定列固定窄列，隐藏移动端判定前缀 `>`，避免「是也不是」与用户名/正文重叠。

对局日志视觉规则：桌面端普通日志正文为 15px、判定标签为 13px、提示/线索/汤底揭晓卡片标题与正文不低于 15px；移动端日志正文约 12px、时间/判定列更紧凑，以保证提问正文宽度。
