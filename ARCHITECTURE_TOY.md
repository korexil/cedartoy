...
| `finished_room_retention_hours` | `24` | 已结束房间后端继续保留小时数；大厅立即隐藏已结束房，到期后删除房间、日志和记事 |
...
- `GET /`：房间列表，含状态、汤面、房间/题库 `title`、题库 `tags`、提问数、活跃玩家数（presence 统计）、最近活跃时间（`last_active_at`，取房间 presence 最大值）。仅返回 `waiting`/`playing` 房间；`finished` 房间会从大厅立即隐藏，但后端按 `finished_room_retention_hours` 继续保留。
- `POST /create`：创建房间。
  - `mode=random`：从题库题创建。
...
- `POST /ask`：提问。body: `{room_id, content}`。
  ...
  - 若触发 `clue`，实际公布内容优先从房间汤底的 `【线索公布】...【线索公布结束】` 包裹段读取，兼容旧格式时最多读到下一段以 `【` 开头的内容；额外写入 `type='auto_hint'`、`judgment='auto_hint'` 并广播 `new_log`（前端绿色横幅标题「【特殊线索】」）。
...
| action | 参数 | 行为 |
| --- | --- | --- |
| `list_rooms` | 无 | 返回 `waiting/playing` 房间列表，字段 `id/title/surface/status/created_at/last_active_at` |
...
- 触发后的特殊线索落库为 `game_logs.type='auto_hint', judgment='auto_hint'`，前端显示为无按钮绿色「【特殊线索】」横幅；ask 数量阈值触发的普通提示同样使用 `type='auto_hint'`，但不写 `judgment='auto_hint'`，前端显示为无按钮粉色「【提示】」横幅。
...
| 海龟汤房间 | `/rooms/create` 或 MCP `create_random` | 猜中、房主关闭、管理员 finish、48 小时无玩家发言自动结束、大厅立即隐藏已结束房、结束后 24 小时删除 | 普通 API 不暴露 `answer` |
...
| 路由 | 页面 | 说明 |
| --- | --- | --- |
| `/soup/` | `Lobby` | 像素风大厅：房间列表仅显示等待/进行中房间，已结束房会立即从大厅隐藏但后端保留 24 小时；汤名搜索与标签筛选、创建房间防重复提交；题库下拉框可选题，也可随机抽题；AI 生成页签有「风格（可选）」下拉，仅显示用户可理解的风格名，生成中禁用生成/创建按钮但不再使用 5 秒倒计时，生成预览分开显示 `title` 与 `surface`；排行榜排除游客，移动端排行榜分类为横向滑块；底部导航含「大厅」入口，可从排行榜/我的回到房间列表；自定义汤底输入框空白时提示 `【线索公布】...【线索公布结束】` 包裹写法；移动端顶栏保留头像/管理员入口，并以图标按钮显示登出 |
...
房间卡片展示房间级 `title`（无则题库 `title`，再无则汤面首句）、汤面两行省略、标签与「提问/在房」统计；列表默认按最近活跃时间排序，0 提问房间排倒数第二（「按活跃排序」按钮重新拉取列表，移动端另有紧凑刷新按钮）。移动端锁高 `100dvh`、仅中间 `.lobby-main` 内滚动；卡片整卡可点进入；创建用 FAB + 固定高度抽屉，「创建」按钮贴底可见。
