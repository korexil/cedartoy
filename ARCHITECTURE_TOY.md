...
- `surface` / `answer` / `tags`：投稿内容；`surface` 上限 500 字，`answer` 上限 3000 字。
- `submitted_by`：投稿人。
...
- `POST /`：管理员新增题（`PuzzleBody`，使用统一题目长度上限：`title` 20、`surface` 500、`answer` 3000、`tags` 100）。
...
- `POST /create`：创建房间。
  ...
  - 自定义创建统一限制：`title` 20 字、`surface` 500 字、`answer` 3000 字、`tags` 100 字。
