...
- `GET /{room_id}`：房间详情，返回 logs 和 notes；附带房间/题库 `title` 与题库 `tags`；普通响应不含 `answer`。notes 含 `username`，游客缺省名为 `游客{player_id}`。
...
- `POST /ask`：提问。body: `{room_id, content}`。
  ...
  - 当 ask 总数 `>= last_hint_at_ask_count + hint_trigger_count` 且无未处理 `hint_offer` 时，后台异步尝试 `generate_hint(surface, answer, logs)`，写入无 `judgment='auto_hint'` 的 `auto_hint` 普通提示日志，更新 `last_hint_at_ask_count`，广播 `new_log`；前端对普通自动提示显示“是否查看”的接受/拒绝按钮，选择仅保存在本地，特殊线索不需要确认；自动提示失败不阻塞本次提问，并记录后端异常日志。
...
- `POST /hint/request`：手动请求提示。body: `{room_id}`。
  ...
  - `generate_hint` 使用裁判 LLM 池，提示格式失败最多重试 3 次；生成失败返回 503；成功则写入带 `player_id`、`resolved=1` 的 `hint_offer`，更新 `last_hint_at_ask_count`，广播 `hint_offer`（前端「【请求提示】」横幅直接显示提示内容，请求前仍二次确认）。
...
| `hint_request` | `room_id`, `path_token?` | 主动请求一次提示并直接返回/显示提示内容，每个玩家每房间最多 3 次；同房间提示生成串行调用裁判 LLM，最多 3 次格式重试；若房间已结束，返回带 `status` 查看指引的错误 |
...
生产 API 池现状应通过管理后台维护：只有 `enabled=1` 的配置参与轮询。格式或语义不稳定的配置即使连通也会影响对局；`judge_ask` 和 `generate_hint` 会通过 `_chat_validated` 按各自重试次数重新调用 `_chat()`，`judge_guess` 在独立猜底锁内要求 JSON 并最多重试 3 次，格式失败会记录 warning 日志。
...
- `generate_hint(surface, answer, game_log) -> str | None`
  ...
  - 使用最近最多 40 条 ask 记录；追加系统消息要求回复以 `【提示】` 开头、总字数不超过 30 字且无标点；后端最终仍做 120 字兜底截断；最多 3 次格式重试；单次 LLM 调用使用较短 timeout 与 `max_tokens=180`；失败返回 `None`。
...
7. `judge_api_configs.api_key` 存在 SQLite 中，管理 API 列表会脱敏，但数据库文件本身需要限制访问权限。启用配置前不只要测 HTTP 连通，还要用真实 `ask/guess/hint_request` 场景确认输出格式与语义；`guess` 现在要求严格 JSON，连通但格式错误的节点会在后端内部重试，连续失败后玩家才会看到 `【系统提示】系统开小差了...`。
