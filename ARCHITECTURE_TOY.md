...
- `title`：房间标题，可为空；题库房创建时复制 `puzzles.title`，自定义/MCP/AI 生成房可直接保存房间级标题。房间列表和详情优先返回 `rooms.title`，再回退题库 `puzzles.title`。
- `surface`：房间汤面。
- `answer`：房间汤底。
...
- `POST /ask`：提问。body: `{room_id, content}`。
  ...
  - 若触发 `clue`，额外写入 `type='auto_hint'`、`judgment='auto_hint'` 并广播 `new_log`（前端绿色横幅标题「【特殊线索】」）。
  - 更新 `players.ask_count` 和对应分项；广播 SSE `new_log`。
  - 当 ask 总数 `>= last_hint_at_ask_count + hint_trigger_count` 且无未处理 `hint_offer` 时，后台异步尝试 `generate_hint(surface, answer, logs)`，写入无 `judgment='auto_hint'` 的 `auto_hint` 普通提示日志，更新 `last_hint_at_ask_count`，广播 `new_log`（前端以无按钮粉色「【提示】」横幅展示）；自动提示失败不阻塞本次提问，并记录后端异常日志。
...
| `create_custom` | `title?`, `surface`, `answer`, `tags?`, `path_token?` | 创建自定义题房间，复用人类端 `create_room(mode="custom")`，走 `scan_text` 审核并写入投稿表；`title` 保存为房间标题 |
| `generate` | `style?` | 调用 `/game/generate` 返回 `title/surface/answer` 预览，不写库、不开房；`style` 支持 `cozy/absurd/mystery/fantasy/history/scifi/horror`，缺省走后端默认风格；满意后再用 `create_custom` |
...
- `generate_puzzle(style="horror") -> {title, surface, answer}`
  - 实时读取 `generate_prompt`；将 `STYLE_DESCRIPTIONS[style]` 通过 `.replace("{style_description}", style_desc)` 注入 prompt，并额外要求返回含 `title/surface/answer` 的 JSON。这里刻意不用 `.format()`，避免 prompt 中 JSON 大括号触发格式化错误。
  - 解析失败返回 HTTP 502。
...
| 路由 | 页面 | 说明 |
| --- | --- | --- |
| `/soup/` | `Lobby` | 像素风大厅：房间列表（含 `finished_room_retention_hours` 内已结束房）、汤名搜索与标签筛选、创建房间防重复提交；题库下拉框可选题，也可随机抽题；AI 生成页签有「风格（可选）」下拉，仅显示用户可理解的风格名，生成中禁用生成/创建按钮但不再使用 5 秒倒计时，生成预览分开显示 `title` 与 `surface`；排行榜排除游客，移动端排行榜分类为横向滑块；底部导航含「大厅」入口，可从排行榜/我的回到房间列表；自定义汤底输入框空白时提示 `【线索公布】` 标记写法；移动端顶栏保留头像/管理员入口，并以图标按钮显示登出 |
| `/soup/room/:id` | `Room` | 对局页：SSE 订阅、`presence.enter_room`；判定前缀中文展示；开小差类日志归一为 `【系统提示】...`；记事本新增时会收到 `judgment='note_notice'` 的系统公屏提示 `【系统提示】记事本有新记录。`，前端以浅灰小字显示；猜测汤底显示为独立玩家猜测卡片与裁判判定卡片；`type='auto_hint'` 且 `judgment='auto_hint'`→绿色「【特殊线索】」，无该 judgment 的 `auto_hint`→粉色「【提示】」；`hint_offer`→粉色「【请求提示】」（每个玩家每房间 3 次，只有请求者能接受/拒绝，请求前确认）；`game_over`→「汤底揭晓」且还原度拆成独立标记；记事本侧栏/抽屉；若 ask/guess 返回 `system_error` 则保留输入框内容；移动端顶栏保留房间状态并以图标按钮显示登出 |
