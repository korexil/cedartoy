...
- `GET /`：房间列表，含状态、汤面、题库 `title`/`tags`、提问数、活跃玩家数（presence 统计）。仅返回 `waiting`/`playing` 房间，以及 48 小时内结束的 `finished` 房间。
- `POST /create`：创建房间。
  - `mode=random`：从题库题创建。
...
- `POST /ask`：提问。body: `{room_id, content}`。
  ...
  - 当 ask 总数 `>= last_hint_at_ask_count + hint_trigger_count` 且无未处理 `hint_offer` 时，后台异步尝试 `generate_hint(surface, answer, logs)`，写入 `auto_hint` 日志，更新 `last_hint_at_ask_count`，广播 `new_log`（前端以无按钮「【线索公布】」横幅展示）；自动提示失败会被吞掉，不阻塞本次提问。
...
- `POST /hint/request`：手动请求提示。body: `{room_id}`。
  ...
  - 调用 `generate_hint`；生成失败返回 503；成功则 `manual_hint_count + 1`，更新 `last_hint_at_ask_count`，广播 `hint_offer`（前端「【请求提示】」横幅，接受/拒绝，请求前二次确认）。
...
- `game=turtle_soup`：返回 action 字典和 notes（含随机题库微恐提示）。
...
| `status` | `room_id`, `log_limit?` | 返回房间公开状态与日志；`log_limit` 取最新 N 条后按时间正序返回；不返回汤底和记事板 |
| `register` | `username`, `password` | 仅注册 Toy AI 账号，返回 path token；让人类把 MCP 地址改为 `https://toy.cedarstar.org/{token}` 后获得持久身份 |
| `create_random` | `path_token?` | 以 MCP 玩家身份创建随机题房间；题库抽取大多微恐 |
...
| `ask` | `room_id`, `content`, `path_token?` | 调用海龟汤 `ask` 逻辑，受 AI 冷却限制；若房间已结束返回「房间已结束，无法继续提问」 |
| `guess` | `room_id`, `content`, `path_token?` | 调用海龟汤 `guess` 逻辑 |
| `hint_request` | `room_id`, `path_token?` | 主动请求一次提示，每局最多 3 次 |
| `hint_respond` | `room_id`, `log_id`, `accept`, `path_token?` | 接受或拒绝提示 |
...
海龟汤 MCP 返回的是 `turtle-soup/backend/mcp_app.py` 的普通 JSON，不是 MCP content envelope；根 `server.py` 会再把该 JSON stringify 成 MCP text content。`status` 和 `join` 都不会返回汤底，`guess` 猜中后的 `game_over` 才会让网页侧收到汤底；`note_list` 独立于 `status`，避免进度查询泄露记事板。
...
生产 API 池现状应通过管理后台维护：只有 `enabled=1` 的配置参与轮询。格式或语义不稳定的配置即使连通也会影响对局，因为 `_chat()` 一旦某配置 HTTP 成功就返回文本，后续解析失败不会自动切到下一个配置。
...
- `generate_hint(surface, answer, game_log) -> str | None`
  - 实时读取 `judge_prompt`，并使用最近最多 40 条 ask 记录；回复须以 `【提示】` 开头，返回最多 120 字；失败返回 `None`。
...
注意：清理游客会删除其日志和记事，这意味着老房间历史可能缺少游客发言；注册/平台账号用户不受该任务影响。
...
| 数据 | 创建 | 清理/结束 | 备注 |
| --- | --- | --- | --- |
| 海龟汤游客 `players` | `/auth/guest` 无 `user_id` 或 MCP 无用户名 | `cleanup_guests` 双条件 48 小时默认 | 清理时连带日志、记事、未结束房间 |
| 海龟汤注册/平台玩家 | `/auth/register`、`/auth/login`、`/auth/guest {user_id}` | 无自动删除 | 管理后台可删玩家 |
| 海龟汤房间 | `/rooms/create` 或 MCP `create_random` | 猜中、房主关闭、管理员 finish、游客清理 | 普通 API 不暴露 `answer` |
...
| 路由 | 页面 | 说明 |
| --- | --- | --- |
| `/soup/` | `Lobby` | 像素风大厅：房间列表（含 48h 内已结束房）、汤名搜索与标签筛选、创建房间防重复提交；随机题库面板提示「题库抽取的大多微恐，请酌情选择」；自定义汤底输入框空白时提示 `【线索公布】` 标记写法 |
| `/soup/room/:id` | `Room` | 对局页：SSE 订阅、`presence.enter_room`；判定前缀中文展示；开小差类日志归一为 `【系统提示】...`；`auto_hint`→「【线索公布】」、`hint_offer`→「【请求提示】」（接受/拒绝，上限 3 次，请求前确认）；`game_over`→「汤底揭晓」且正文含还原度；记事本侧栏/抽屉；若 ask/guess 返回 `system_error` 则保留输入框内容 |
...
对局日志视觉规则：普通日志正文为 15px；判定标签为 13px；提示/线索/汤底揭晓卡片标题与正文不低于 15px，避免系统信息比玩家正文弱。
...
7. `judge_api_configs.api_key` 存在 SQLite 中，管理 API 列表会脱敏，但数据库文件本身需要限制访问权限。启用配置前不只要测 HTTP 连通，还要用真实 `ask/guess` 场景确认输出格式与语义；连通但格式错误的节点会造成玩家看到 `【系统提示】系统开小差了...`。
8. `settings.judge_prompt`、`settings.generate_prompt`、`settings.judge_prompt_clue` 不走缓存；通过管理后台或直接改表后，下一次裁判/生成调用立即生效。
