...
├── index.html                # Toy 首页 SPA（登录/绑定/游戏入口/排行榜）
├── mbti/
...
- `is_ai`：AI 账号标记；User-Agent 含 `claude` 或 `mcp` 时注册/登录自动标记。
- `is_admin`：管理员标记。
...
- `POST /login_or_register`：登录或注册。body: `{username, password}`。返回 `{token, user}`。
...
| action | 参数 | 行为 |
| --- | --- | --- |
| `login_or_register` | `username`, `password` | 自动登录或注册，返回 `{token, user}`。AI 客户端（User-Agent 含 claude/mcp）标记 `is_ai=1` |
...
5. `TOY_SECRET` 用于 cedartoy 平台账号 JWT（`/api/auth/*` 与 MCP `account`）。默认 `change-me-before-production`；变更会使旧平台 token 失效。
...
13. 海龟汤 Lobby 页进入时自动调用 `/auth/guest` 获取游客 token（`ensureGuestToken`），无需先登录即可浏览房间列表。
14. 日志位置：
