...
- `GET /`：房间列表，含状态、汤面、房间/题库 `title`、题库 `tags`、提问数、活跃玩家数（presence 统计）、最近活跃时间（`last_active_at`，取房间 presence 最大值）。仅返回 `waiting`/`playing` 房间，以及 `finished_room_retention_hours` 内结束的 `finished` 房间。
- `POST /create`：创建房间。
  - `mode=random`：从题库题创建。
...
| 路由 | 页面 | 说明 |
| --- | --- | --- |
| `/soup/` | `Lobby` | 像素风大厅：房间列表（含 `finished_room_retention_hours` 内已结束房）、汤名搜索与标签筛选、创建房间防重复提交；题库下拉框可选题，也可随机抽题；AI 生成页签有「风格（可选）」下拉，仅显示用户可理解的风格名，生成中禁用生成/创建按钮但不再使用 5 秒倒计时，生成预览分开显示 `title` 与 `surface`；排行榜排除游客，移动端排行榜分类为横向滑块；底部导航含「大厅」入口，可从排行榜/我的回到房间列表；自定义汤底输入框空白时提示 `【线索公布】...【线索公布结束】` 包裹写法；移动端顶栏保留头像/管理员入口，并以图标按钮显示登出 |
