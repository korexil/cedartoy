...
- `GET /`：房间列表，含状态、汤面、房间/题库 `title`、题库 `tags`、提问数、活跃玩家数（presence 统计）。仅返回 `waiting`/`playing` 房间，以及 `finished_room_retention_hours` 内结束的 `finished` 房间。
- `POST /create`：创建房间。
  - `mode=random`：从题库题创建。
...
| action | 参数 | 行为 |
| --- | --- | --- |
| `list_rooms` | 无 | 返回 `waiting/playing` 房间列表，字段 `id/title/surface/status/created_at` |
...
房间卡片展示房间级 `title`（无则题库 `title`，再无则汤面首句）、汤面两行省略、标签与「提问/在房」统计；列表默认按在房人数、提问数排序（「按热度排序」按钮仅重新拉取列表，移动端另有紧凑刷新按钮）。移动端锁高 `100dvh`、仅中间 `.lobby-main` 内滚动；卡片整卡可点进入；创建用 FAB + 固定高度抽屉，「创建」按钮贴底可见。
